...
def scrape_aaa():
    ...
    return {"currentRegular": row("Current Avg."), "weekAgoRegular": row("Week Ago Avg."), "monthAgoRegular": row("Month Ago Avg."), "yearAgoRegular": row("Year Ago Avg."), "sacramentoRegular": clean_number(metro.group(1)) if metro else None, "sacramentoWeekAgoRegular": clean_number(metro.group(2)) if metro else None}
def fetch_eia_history(api_key: str | None):
    ...
    if not reg_rows or not diesel_rows:
        if existing.get("series"): return {**existing, "lastUpdated": now_iso(), "notes": (existing.get("notes", []) + ["EIA returned no rows on this run. Preserved previous history."])[-5:]}
    ...
    if not months:
        if existing.get("series"): return {**existing, "lastUpdated": now_iso(), "notes": (existing.get("notes", []) + ["EIA returned no overlapping gasoline/diesel periods. Preserved previous history."])[-5:]}
        raise RuntimeError("EIA returned no overlapping gasoline/diesel periods.")
    return {"lastUpdated": now_iso(),"sourceStrength":"High","freshness":"Monthly","sourceLinks":[{"label":"EIA","url":"https://www.eia.gov/"}],"notes":["History refreshed from EIA."],"series":[{"date":m,"regular":reg_map[m],"diesel":diesel_map[m]} for m in months]}
aaa = scrape_aaa()
