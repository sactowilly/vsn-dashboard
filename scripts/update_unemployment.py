...
def fetch_series(series_id: str, api_key: str, limit: int = 12):
    ...
    r.raise_for_status()
    rows = r.json().get("observations", []); rows.reverse()
    return [{"date": o["date"][:7], "value": float(o["value"])} for o in rows if o.get("value") not in (".", None)]
api_key = env("FRED_API_KEY")
if not api_key:
    write_json("unemployment.json", preserve_with_failure("unemployment.json", "FRED_API_KEY is required for update_unemployment.py. Preserved previous unemployment data.", source_type="automated", limitations=["Monthly BLS/FRED series can lag the current operating week."]))
    raise SystemExit(0)
...
if not sac_id:
    ...
    r.raise_for_status(); items = r.json().get("seriess", [])
    if not items: raise SystemExit("Could not discover Sacramento unemployment series from FRED search.")
    sac_id = items[0]["id"]
us = fetch_series("UNRATE", api_key); ca = fetch_series("CAUR", api_key); sac = fetch_series(sac_id, api_key)
by_month = {}
